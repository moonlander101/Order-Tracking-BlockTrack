...
peer lifecycle chaincode package ordercc.tar.gz \
...
peer lifecycle chaincode install ordercc.tar.gz
...
PACKAGE_ID=$(peer lifecycle chaincode queryinstalled | grep "Package ID" | awk '{print $3}' | sed 's/,$//')
